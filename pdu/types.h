...
namespace cuap::pdu
{
   ...
   enum class CommandIDs
   {
      ...
      Begin  = 0x0000006f, Contniue = 0x00000070, End = 0x71, Abort = 0x72,
   ...
   enum BindBody /** 11, 9, 13, 4 bytes respectively*///  : uint8_t
   ...
   */
   ...
   struct basic_header : public misc::static_buffer<T, SZ>
   {
      ...
      uint32_t command_len()    { return this->get_u32(Header::CommandLength); }
      constexpr uint32_t command_id()     { return this->get_u32(Header::CommandID); }
      uint32_t command_status() { return this->get_u32(Header::CommandStatus); }
      uint32_t sender_id()      { return this->get_u32(Header::SenderID); }
      uint32_t receiver_id()    { return this->get_u32(Header::ReceiverID); }
   ...
   using header_msg = basic_header<uint8_t, 20>;
   using abort_msg  = basic_header<uint8_t, 20>;
}
